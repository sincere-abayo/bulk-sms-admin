...
const Dashboard: React.FC = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      const response = await fetch('http://localhost:4000/api/auth/admin/dashboard-stats', {
        headers: {
          ...
          {trend && (
            <div className="flex items-center">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                {trendType === 'positive' && <FiArrowUp className="w-3 h-3 mr-1" />}
                {trendType === 'negative' && <FiArrowDown className="w-3 h-3 mr-1" />}
                {trend}
              </span>
            </div>
          )}
    ...
    <div className={`p-4 md:p-6 space-y-6 bg-background text-foreground`}>
      ...
      <div className="flex flex-col space-y-4 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
          ...
          <StatCard
            title="Total Revenue"
            value={`${stats?.currency || 'RWF'} ${(stats?.totalRevenue || 0).toLocaleString()}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
        ...
        <div className="bg-card rounded-xl shadow-sm p-6 border border-border">
          ...
          <div className="space-y-4">
            {[
              { id: 1, user: 'John Doe', action: 'sent a campaign', time: '2 min ago', status: 'success' },
              { id: 2, user: 'Jane Smith', action: 'topped up balance', time: '15 min ago', status: 'success' },
      ...
      <div className="bg-card rounded-xl shadow-sm p-6 border border-border mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-foreground">
          {[
            ...
            { service: 'API Services', status: 'operational', uptime: '99.7%', responseTime: '89ms' },
            { service: 'Payment Gateway', status: 'operational', uptime: '99.5%', responseTime: '156ms' },
          ].map((service, index) => (
